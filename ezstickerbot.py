...
from requests.exceptions import ConnectionError, ConnectTimeout, HTTPError, InvalidURL, RequestException, Timeout
...
def restricted(func):
    ...
    def wrapper(update, context):
        ...
        if validate_chat_id(chat_id):
            ...
            logging.info(f"Unauthorized access denied for {chat_id} ({update.message.from_user.name})")  # noqa: E501
...
def video_received(update: Update, context: CallbackContext):
    ...
    try:
        ...
        sent_message = message.reply_document(
            document=document,
            filename=filename,
            caption=get_message(user_id, "forward_to_stickers"),
            quote=True,
            timeout=30,
        )
        # add a keyboard with a forward button to the document
...
def make_video(input_file):
    ...
    # Calculate target bitrate to stay under 256KB for 3 seconds
    # 256KB = 262144 bytes, accounting for overhead use 240KB target
    # bitrate (in kbits/s) = (file_size_in_bytes * 8) / duration_in_seconds / 1000
    target_bitrate = int((240 * 1024 * 8) / 3 / 1000)  # approximately 640k
    ...
    cmd = [
        ...
        "00:00:03",
        "-filter:v",
        "fps=fps=30,scale={}:{}".format(new_width, new_height),
        "-an",
        "-c:v",
        "libvpx-vp9",
        "-b:v",
...
def create_sticker_file(message, image, context: CallbackContext):
    ...
    # else format image to sticker
        ...
        if new_width % 1 >= 0.999:
            new_width = int(round(new_width))
        else:
            new_width = int(new_width)
        if new_height % 1 >= 0.999:
        ...
        if word[0] == "$":
            try:
                ...
                message[i] = '<a href="tg://user?id={}">{}{}</a>'.format(
                    _id, user.first_name, " " + user.last_name if user.last_name else ""
                )
            except ValueError:
...
def donate_command(update: Update, context: CallbackContext):
    ...
    message_text = get_message(
        ...
        config["donate_paypal"], config["donate_cashapp"], config["donate_btc"], config["donate_eth"]
    )
...
def icon_command(update: Update, context: CallbackContext):
    ...
    keyboard = [[InlineKeyboardButton(get_message(message.chat_id, "cancel"), callback_data="icon_cancel")]]
    markup = InlineKeyboardMarkup(keyboard)
...
def info_command(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton(get_message(message.chat_id, "source"), url=config["source_link"]),
        ],
        [
            InlineKeyboardButton(get_message(message.chat_id, "rate"), url=config["rate_link"]),
...
def lang_stats_command(update: Update, context: CallbackContext):
    ...
    for code, count in sorted_usage:
        lang_stats_message += "\n" + "\u200e" + "{}: {:,}".format(lang[code]["lang_name"], count)
...
def broadcast_thread(context: CallbackContext):
    ...
    if context.job.context is None:
        logging.info("Broadcast thread created without message stored in job context")
        return
    ...
    for user_id in list(users):
        ...
        try:
            if opt_in and not config["override_opt_out"]:
                bot.send_message(
                    chat_id=int(user_id), text=context.job.context, parse_mode="HTML", disable_web_page_preview=True
                )
